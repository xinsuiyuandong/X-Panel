...
![34](./media/34.png)
#### 6、在做【ssh转发】过程中，本地电脑的终端不能关闭，保持打开不能断开；且每一次要登录3X-UI管理后台都要做【ssh转发】，关闭之后就失效了。
![35](./media/35.png)
#### PS：上述两种方法：【ssh端口转发】或申请安装证书的目的都是为了更安全地登录面板，而至于搭建的其他流程和步骤，都是一样的；如果你已经【申请安装证书】了，并不会受到其他什么额外影响，就不用去折腾【ssh转发】了，直接用 【http://你的域名:端口/路径】 去登录你的面板管理后台就行了。
